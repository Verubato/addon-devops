#!/usr/bin/env python
"""Enforces the Mini addons' file-layout and load-order conventions.
...
Shared across every Mini addon. Run from an addon's repository root, or point it at one:

    python build/CheckConventions.py
    python build/CheckConventions.py ../MiniHealthNumbers
    python build/CheckConventions.py --self-test

An addon with no src/ directory is reported as nothing to check rather than as a failure.
